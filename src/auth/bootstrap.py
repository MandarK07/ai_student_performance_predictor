...
def decommission_legacy_demo_users_on_startup() -> None:
    """
    Decommission legacy public 'teacher_demo' / 'student_demo' accounts that
    previously lived in the production schema. Demo users now exist ONLY inside
    the isolated 'demo' schema created by ensure_demo_ready().
    """
    from src.database.demo import decommission_legacy_demo_users
    db = SessionLocal()
    try:
        decommission_legacy_demo_users(db)
    finally:
        db.close()
